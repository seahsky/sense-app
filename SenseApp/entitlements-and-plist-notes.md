# SenseApp (iOS target) — Info.plist / entitlements notes

Written for whoever owns `project.yml` / target scaffolding. This iOS app target's
Swift sources are implemented under `SenseApp/` (App entry point, `Views/`,
...
The architecture spec's §6 says `SenseApp/Info.plist` needs
...
`WorkoutSessionManager`) lives exclusively in `SenseWatch/Workout/` per the spec's
...
`HKHealthStore` call anywhere in `SenseApp/`.

**So for the current scope: no HealthKit Info.plist keys, no HealthKit capability,
no `com.apple.developer.healthkit` entitlement on the `SenseApp` target.**
...
- `NSHealthShareUsageDescription` (read) and, only if the phone ever writes samples
  itself, `NSHealthUpdateUsageDescription` (write) to `SenseApp/Info.plist`.
- The "HealthKit" capability on the `SenseApp` target in Xcode (adds
...
`SenseApp.swift` calls `WatchConnectivityBridge.shared.activate()` at launch
...
target: SenseWatch, embed: true` relationship in `project.yml`, §6 of the spec).
...
contacts, or notification usage appears anywhere in `SenseApp/`'s implemented
Swift code — none of those keys are needed on this target.

## Summary

| Item | Needed on `SenseApp` target? |
|---|---|
...
| `WKBackgroundModes` / "Workout Processing" | N/A — that's a `SenseWatch/Info.plist` concern only, not this target's |
